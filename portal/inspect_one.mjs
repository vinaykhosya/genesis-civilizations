...
for (const [id, folderName] of Object.entries(mapping)) {
  ...
  if (fs.existsSync(configPath)) {
    ...
    console.log(
      `Config: ticks=${config.ticks}, seed=${config.seed}, scarcity=${config.scarcity}, preset=${config.world_preset}`,
    );
  } else {
  ...
  if (fs.existsSync(summaryPath)) {
    ...
    console.log(
      `Summary: ticks=${summary.ticks}, seed=${summary.seed}, max_generation=${summary.max_generation}, survivors=${summary.survivors}`,
    );
  } else {
