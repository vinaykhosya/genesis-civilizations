...
export async function publishExperiment(
  ...
  const thumbnailUrl = hasWorldImage ? `${storageUrlPrefix}/experiments/${paths.thumbnailWebpPath}` : null;
  ...
  if (canonical.elevationPngBytes && canonical.elevationPngBytes.length > 0) availableMaps.push("elevation");
  if (canonical.temperaturePngBytes && canonical.temperaturePngBytes.length > 0) availableMaps.push("temperature");
  if (canonical.rainfallPngBytes && canonical.rainfallPngBytes.length > 0) availableMaps.push("rainfall");
  if (canonical.riversPngBytes && canonical.riversPngBytes.length > 0) availableMaps.push("rivers");
  if (canonical.habitabilityPngBytes && canonical.habitabilityPngBytes.length > 0) availableMaps.push("habitability");
  if (canonical.tradePngBytes && canonical.tradePngBytes.length > 0) availableMaps.push("trade");
  if (canonical.simulationPngBytes && canonical.simulationPngBytes.length > 0) availableMaps.push("simulation");
