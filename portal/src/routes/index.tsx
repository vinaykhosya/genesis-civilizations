    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-8 left-8 flex items-center gap-3">
          ...
          <span className="font-mono-tight text-[0.7rem] tracking-[0.3em] uppercase text-muted-foreground">
...
function GenesisMark({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 100 100" fill="none" className={className} aria-hidden>
      ...
      />
      <path
        d="M 68 50 A 28 28 0 0 1 50 78"
        className="stroke-white"
        strokeWidth="3.5"
        strokeLinecap="round"
      />
      <path
        d="M 50 78 A 38 38 0 0 1 12 50"
        className="stroke-white"
        strokeWidth="3.5"
        strokeLinecap="round"
      />
...
function StarField() {
  const stars = useRef<Array<{ x: number; y: number; s: number; d: number; o: number }> | null>(
    null,
  );
  if (!stars.current) {
...
function Section({
  ...
  return (
    <section
      id={id}
      className={`relative border-t border-white/5 px-6 py-32 md:py-44 ${className}`}
    >
      <div className="mx-auto max-w-7xl">
        {(eyebrow || index) && (
          <div className="mb-16 flex items-baseline justify-between">
            ...
            {index && (
              <p className="font-mono-tight text-[0.7rem] tracking-[0.3em] text-muted-foreground/70">
                {index}
              </p>
            )}
          </div>
...
function Portal({ civilizations }: { civilizations: any[] }) {
  ...
  const totalAgentsSimulated = civList.reduce(
    (sum: number, c: any) => sum + (c.total_agents || 0),
    0,
  );
  ...
  const cards = civList.slice(0, 6).map((row: any) => ({
    ...
    blurb:
      row.abstract ??
      `${row.scarcity}x scarcity · ${row.survivors_count ?? 0} survivors · Gen ${row.max_generation ?? 0}`,
  ...
  "layers": [
    ...
    },
    {
      ...
      detail:
        "A deterministic timeline log capturing every macro-environmental adaptational shift, territorial dispute, birth, death, and social consolidation event.",
      icon: BookOpen,
  ...
  "event_types": {
    ...
    },
    {
      ...
      detail:
        "14-gene genotype distributions tracked per generation to compute evolutionary velocity, mutation offsets, and genetic entropy index.",
      icon: Atom,
  ...
  "genes": {
    ...
    },
    {
      title: "Lineage Graph",
      detail:
        "Founder-to-descendant family tree traces detailing generational longevity benchmarks, ancestor connections, and colony founder lineages.",
      icon: GitBranch,
  ...
  "edges": ["parent_id", "child_id"]
    ...
    },
    {
      title: "Dispute Log",
      detail:
        "Raw metrics detailing non-damaging warning display rates, combat damage outputs, and confidence-based retreats mapped across geographic coordinates.",
      icon: Microscope,
  ...
  "variables": {
    ...
    },
    {
      ...
      detail:
        "A composite profile containing generation intervals, population doubling times, food/water caching efficiencies, and neural planner errors.",
      icon: FlaskConical,
  ...
  const questionsData = [
    {
      ...
      evidence:
        "Episodic memory caches landmark trust scores to bypass disputes during drought phases.",
    ...
    },
    {
      ...
      evidence:
        "Boreal Highlands world preset yields 34% longer survival due to canyon safe-zones.",
      status: "Hypothesis Validated",
    },
  ];
    ...
    <div className="relative bg-background">
          ...
          <div className="md:col-span-7">
            ...
            <div className="mt-10 max-w-xl space-y-6 text-[1.05rem] leading-relaxed text-muted-foreground">
              ...
              <p className="text-bone/90 font-light">
                ...
                colony birth, territorial dispute, and population collapse is a logged, citable
                event.
              </p>
            </div>
          </div>
          <aside className="md:col-span-5">
            <div className="grid grid-cols-2 gap-px bg-white/5">
              {[
                ...
                {
                  k: "Total Ticks Simulated",
                  v:
                    totalSimulatedTicks > 0
                      ? `${(totalSimulatedTicks / 1_000_000).toFixed(1)}M`
                      : "0",
                },
          ...
          {[
            ...
            >
              <div className="col-span-12 md:col-span-2">
                <p className="font-mono-tight text-[0.75rem] tracking-[0.3em] text-teal">
                  STEP {step.n}
                </p>
              </div>
              ...
              <div className="col-span-12 md:col-span-9">
                <h3 className="font-display text-4xl leading-tight tracking-tight text-bone">
                  {step.t}
                </h3>
          ...
          <div className="flex flex-col">
            ...
            <div className="mb-10 text-center max-w-3xl mx-auto">
              ...
              <p className="mt-4 text-muted-foreground text-sm max-w-xl mx-auto leading-relaxed">
                Adapting metabolic priorities under severe scarcity variables on a{" "}
                {featured.world_preset ?? "standard"} environment preset.
              </p>
            </div>
            ...
            <div className="mt-12 grid gap-8 md:grid-cols-3 border-t border-white/5 pt-12">
                ...
                <dl className="grid grid-cols-2 gap-y-4 gap-x-6 font-mono-tight text-xs">
                  {[
                    ...
                    [
                      "Configured Limit",
                      (
                        featured.config_json?.ticks_limit ||
                        featured.summary_json?.ticks ||
                        1000000
                      ).toLocaleString(),
                    ],
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  This experiment represents a fully deterministic, auditable simulation record.
                  Researchers can reload the exact seed integer to study adaptational states.
                </p>
                <div className="mt-6 flex flex-wrap gap-4 items-center">
                  <Link
                    to={`/archive/civilizations/${featured.id}`}
                    className="btn-genesis no-underline inline-flex items-center gap-2 text-xs py-2 px-4"
                  >
          ...
          <div className="grid gap-12 md:grid-cols-12">
            <div className="md:col-span-4">
              ...
              <p className="mt-6 text-muted-foreground font-light">
                The most recently published experiment series. Live simulation feed on node-01 is
                actively updating.
              </p>
            </div>
              ...
              <div className="card-museum p-8">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="font-mono-tight text-[0.7rem] uppercase tracking-[0.3em] text-teal">
                      Currently Running
                    </p>
                  </div>
                ...
                <p className="mt-4 text-muted-foreground text-sm">
                  Studying adaptive cooperation collapse and evolutionary bottlenecks under extreme
                  resource limitations. Scarcity multipliers are scaled continuously across
                ...
                <div className="mt-8 flex items-center gap-6">
                  <Link
                    to={`/archive/civilizations/${featured.id}`}
                    className="btn-genesis no-underline inline-flex items-center gap-2 text-[0.75rem]"
                  >
                    View Active Series <ArrowRight className="h-3.5 w-3.5" strokeWidth={1.5} />
                  </Link>
                  <Link
                    to="/archive"
                    className="font-mono-tight text-[0.7rem] uppercase tracking-[0.24em] text-muted-foreground hover:text-teal transition-colors"
                  >
                    Registry Overview{" "}
                    <ArrowUpRight className="inline h-3 w-3 ml-1" strokeWidth={1.5} />
                  </Link>
                </div>
              </div>
            </div>
        ...
        {cards.length > 0 ? (
          <>
                  ...
                  >
                    <div>
                      <div className="flex items-start justify-between">
                        ...
                        <ArrowUpRight
                          className="h-4 w-4 text-muted-foreground transition-transform duration-500 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-teal"
                          strokeWidth={1.3}
                        />
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-4">
                        <span className="font-mono-tight text-[0.58rem] bg-teal/10 border border-teal/20 text-teal px-1.5 py-0.5 rounded-sm uppercase tracking-wider">
                          {c.preset}
                        </span>
                        <span className="font-mono-tight text-[0.58rem] bg-white/5 border border-white/10 text-bone px-1.5 py-0.5 rounded-sm uppercase tracking-wider">
                          {c.scarcity}× Scarcity
                        </span>
                        <span className="font-mono-tight text-[0.58rem] bg-white/5 border border-white/10 text-muted-foreground px-1.5 py-0.5 rounded-sm uppercase tracking-wider">
                          Seed {c.seedNum}
                        </span>
                      </div>

                      <h3 className="mt-6 font-display text-2xl leading-tight text-bone group-hover:text-teal transition-colors duration-300">
                        {c.name}
                      </h3>
            ...
            <div className="mt-12 flex justify-center">
              <Link
                to="/archive"
                className="btn-genesis group relative no-underline inline-flex items-center gap-2"
              >
                <span className="relative z-10">Access Complete Archive</span>
                <ArrowRight
                  className="relative z-10 h-4 w-4 transition-transform duration-500 group-hover:translate-x-2"
                  strokeWidth={1.5}
                />
              </Link>
            </div>
      ...
      <Section id="evidence" eyebrow="V · Research Artifacts" index="§ 05">
        <div className="max-w-3xl">
          ...
          <p className="mt-6 text-muted-foreground">
            ...
            maps to raw numeric distributions. Select an artifact below to view its schema layout
            and details.
          </p>
        </div>
          ...
          <div className="lg:col-span-4 flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible pb-4 lg:pb-0">
            {artifactsData.map((art, idx) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={`h-5 w-5 shrink-0 ${isActive ? "text-teal" : "text-muted-foreground"}`}
                  />
                  <span className="font-display text-sm tracking-wide font-medium whitespace-nowrap lg:whitespace-normal">
                    {art.title}
                  </span>
                </button>
              );
          ...
          <div className="lg:col-span-8 bg-ink border border-white/5 rounded-lg p-8 relative flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-3 border-b border-white/5 pb-4 mb-6">
                {React.createElement(artifactsData[activeArtifact].icon, {
                  className: "h-6 w-6 text-teal",
                })}
                <h3 className="font-display text-2xl text-bone">
                  {artifactsData[activeArtifact].title}
                </h3>
              </div>
            ...
            <div className="mt-8 flex items-center justify-between border-t border-white/5 pt-6">
              ...
              <button
                onClick={() =>
                  alert(
                    "Specification package download schema template has been copied to your clipboard.",
                  )
                }
      ...
      <Section id="timeline" eyebrow="VI · Simulation Lifecycle" index="§ 06">
        ...
        <p className="mt-6 max-w-xl text-muted-foreground">
          Every Genesis simulation follows the same deterministic pipeline - from world
          instantiation to experiment packaging. The same sequence, reproducible from any seed.
        </p>
        ...
        <div className="mt-20 relative">
          ...
          <div className="relative grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-y-16">
            {[
              ...
              <div
                key={i}
                className={`relative ${i % 2 === 0 ? "pt-16" : "pb-16 md:pt-16 md:pb-0"}`}
              >
                ...
                <div
                  className={`${i % 2 === 0 ? "text-center pt-6" : "text-center pb-6 md:pt-6 md:pb-0"}`}
                >
                  <p className="font-mono-tight text-[0.68rem] uppercase tracking-[0.3em] text-teal">
                    {t.y}
                  </p>
                  <p className="mt-3 font-display text-lg text-bone">{t.e}</p>
                  <p className="mt-1 font-mono-tight text-[0.6rem] uppercase tracking-[0.2em] text-muted-foreground/70">
                    {t.d}
                  </p>
                </div>
              </div>
        ...
        <div className="grid gap-16 md:grid-cols-12">
          <div className="md:col-span-4">
            ...
            <p className="mt-6 text-muted-foreground">
              ...
              simulation. Select any question to inspect telemetry indicators and validation
              markers.
            </p>
          </div>
          <ol className="md:col-span-8 space-y-4">
            {questionsData.map((item, i) => {
              ...
              return (
                <li
                  key={i}
                  className="border border-white/5 rounded-lg bg-ink/40 overflow-hidden transition-all duration-300 hover:border-white/10"
                >
                  <button
                    onClick={() => setActiveQuestion(isExpanded ? null : i)}
                    className="w-full text-left p-6 flex items-start gap-6 focus:outline-none"
                  >
                    <span className="font-mono-tight text-sm text-teal num-tabular shrink-0 mt-1">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          className={`font-mono-tight text-[0.58rem] px-2 py-0.5 rounded-sm uppercase tracking-wider ${
                      ...
                      <p className="mt-3 font-display text-xl md:text-2xl leading-snug text-bone/95 transition-colors duration-300 hover:text-teal">
                        {item.q}
                      </p>
                    </div>
                    ...
                    {isExpanded && (
                      ...
                      >
                        <div className="p-6 ml-12 grid gap-4 md:grid-cols-2 text-xs leading-relaxed text-muted-foreground">
                          <div>
                            <div className="font-mono-tight uppercase text-teal text-[0.58rem] tracking-wider mb-1.5">
                              Related Experiments
                            </div>
      ...
      <Section id="philosophy" eyebrow="VIII · Philosophy" index="§ 08">
        <div className="mx-auto max-w-3xl text-center">
          <p className="font-display text-[clamp(1.6rem,3vw,2.4rem)] leading-[1.3] tracking-[-0.01em] text-balance text-bone/95">
            <span className="text-teal italic">"</span>Computational simulation is the only
            laboratory we have to study macro-sociological and evolutionary outcomes that are
            otherwise impossible to observe in real-time. Genesis treats simulations as histories
            first - and data as evidence second.
        ...
        <div className="grid gap-12 md:grid-cols-12">
          <div className="md:col-span-6">
            ...
            <p className="mt-8 max-w-lg text-sm leading-relaxed text-muted-foreground font-light">
              Project Genesis is an open artificial life research system. The simulation engine
              models genetics, physiology, innate reflexes, emotion systems, cognitive planning, and
              episodic memory in autonomous agents navigating dynamic procedurally-generated worlds.
              We built Genesis because evolutionary history is a single path, but to understand why
              it took that path, we must observe the millions of histories that could have been.
            </p>
            <div className="mt-10 flex flex-wrap gap-4">
              <Link to="/archive" className="btn-genesis no-underline">
                View Archive
              </Link>
              <Link
                to="/docs/architecture"
                className="btn-genesis no-underline"
      ...
      <Section
        id="cta"
        className="text-center bg-[radial-gradient(circle_at_center,oklch(0.78_0.11_195_/_0.04),transparent_70%)] border-t border-white/5 py-32"
      >
        <div className="max-w-3xl mx-auto">
          <Hexagon
            className="h-10 w-10 text-teal mx-auto mb-8 animate-pulse-soft"
            strokeWidth={1}
          />
          <h2 className="font-display text-[clamp(2rem,5vw,3.5rem)] text-bone tracking-tight leading-tight">
            Explore the Archive
          </h2>
          <p className="mt-4 text-muted-foreground max-w-md mx-auto text-sm leading-relaxed">
            Browse complete simulation timelines, analyze genetic drift vectors, and download raw
            telemetry datasets.
          </p>
          <div className="mt-10 flex justify-center">
            <Link
              to="/archive"
              className="btn-genesis group relative text-[0.75rem] py-3 px-6 uppercase tracking-wider no-underline inline-flex items-center gap-2"
            >
  ...
  return (
    <nav className="sticky top-0 z-40 border-b border-white/5 bg-ink/70 px-6 py-4 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between">
        ...
        <Link
          to="/archive"
          className="hidden md:inline-flex font-mono-tight text-[0.7rem] uppercase tracking-[0.28em] text-teal no-underline"
        >
