...
export async function publishExperiment(
  ...
  const storageUrlPrefix = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public`;
  const thumbnailUrl = `${storageUrlPrefix}/thumbnails/${paths.thumbnailWebpPath}`;
  const ogUrl = `${storageUrlPrefix}/thumbnails/${paths.ogWebpPath}`;
  const coverUrl = `${storageUrlPrefix}/thumbnails/${paths.worldPngPath}`;
